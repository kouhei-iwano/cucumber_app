...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        return render(request, "cucumber_app/index.html")


def showall(request):
    images = Image.objects.all().order_by("-pk")
    context = {"images": images}
    return render(request, "cucumber_app/showall.html", context)
...
def upload(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            img.save()
            return redirect("cucumber_app:result")
    else:
        form = ImageForm()
    context = {"form": form}
    return render(request, "cucumber_app/upload.html", context)

def result(request):
    images = Image.objects.all().order_by("-pk")
    context = {"images": images[1:], "now_image": images[0]}
    return render(request, "cucumber_app/result.html", context)